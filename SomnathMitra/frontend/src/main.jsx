...
function App() {
  ...
  async function ask(question) {
    ...
    try {
      const response = await fetch(`${API_URL}/qa?q=${encodeURIComponent(q)}`)
      if (!response.ok) throw new Error(`The guide returned an error (${response.status}).`)
      const data = await response.json()
      setMessages(prev => [...prev, { role: 'assistant', text: data.reply, data }])
    } catch (err) {
      setError(`${err.message} Check that the GPU backend is running and reachable.`)
