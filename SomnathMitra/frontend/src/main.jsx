...
function App() {
  ...
  return <div className="app-shell">
    <aside className={`sidebar ${mobileMenu ? 'mobile-open' : ''}`}>
      <div className="brand"><div className="brand-mark">ॐ</div><div><div className="brand-name">Shivoham <span>| शिवोऽहम्</span></div><div className="brand-subtitle">Your pilgrimage companion</div></div><button className="mobile-close" onClick={() => setMobileMenu(false)}><X size={18} /></button></div>
      <div className="sidebar-content">
        ...
        <div className="nav-item active"><Compass size={17} /> Ask Shivoham</div>
        <div className="nav-item"><BookOpen size={17} /> Temple guide</div>
        <div className="nav-item"><CircleHelp size={17} /> Help &amp; information</div>
      </div>
      <div className="sidebar-footer"><div className="status-dot" /><div><strong>Connected to Shivoham</strong><small>GPU backend · online</small></div><div className="online-pulse" /></div>
    </aside>
...
function Welcome({ onAsk }) { return <div className="welcome"><div className="welcome-symbol">ॐ</div><div className="eyebrow">WELCOME TO SHIVOHAM</div><h1>How can I help<br /><em>your journey?</em></h1><p className="welcome-copy">I’m Shivoham — your thoughtful guide to the temple,<br className="desktop-only" /> the town, and the road to Shivoham.</p><div className="example-grid">{examples.map(example => <button key={example.text} onClick={() => onAsk(example.text)}><span className="example-icon">{example.icon}</span><span>{example.text}</span><ArrowUp size={15} /></button>)}</div></div> }
  ...
  return (
    <div className={`message-row ${isUser ? 'user-row' : ''}`}>
      ...
      <div className="message-body">
        <div className="message-meta">{isUser ? 'You' : 'Shivoham'} <span>·</span> {isUser ? 'Just now' : 'Shivoham guide'}</div>
...
function Details({ data, onClose }) { return <div className="details-overlay" onClick={onClose}><section className="details-panel" onClick={e => e.stopPropagation()}><div className="details-heading"><div><div className="eyebrow">RESPONSE DETAILS</div><h2>How Shivoham found this</h2></div><button onClick={onClose}><X size={18} /></button></div><div className="metric-row"><div><Clock3 size={16} /><span>Response time</span><strong>{data.elapsed_ms} ms</strong></div><div><Compass size={16} /><span>Intent</span><strong>{data.intent?.intent || 'General guide'}</strong></div></div><div className="details-section"><h3>Sources</h3>{data.sources?.length ? data.sources.map(source => <a className="source" key={source.chunk_id} href={source.page_url || '#'} target="_blank" rel="noreferrer"><BookOpen size={15} /><span>{source.heading || 'Shivoham knowledge base'}<small>Relevance score {source.score}</small></span><ExternalLink size={14} /></a>) : <p className="muted">This answer was generated from Shivoham’s general knowledge.</p>}</div></section></div> }
